...
<p>
  <img alt="Version" src="https://img.shields.io/badge/version-v1.2.0-3b6ef5">
...
> **画师库数据说明 / Data Notice**: 画师库数据（画师名录、收录数、分类与定性描述）由 AI 从 Danbooru 公开数据**爬取与归纳生成，可能存在缺失或判断不准确**，仅供参考，请以 Danbooru 官方页面为准。数据文件是**可选的**：插件会在 `custom_nodes/Gallery4ComfyUI/data/`（或环境变量 `GALLERY4_DATA_DIR` 指定的目录）查找 `artists.jsonl`、`*.classified.csv`、`artlib_specialty.jsonl` 等文件，缺失时画师库自动降级为空（不影响其它功能）。
...
custom_nodes/Gallery4ComfyUI/data/        # 可选：画师库数据（发布包不含，需自行放入）
